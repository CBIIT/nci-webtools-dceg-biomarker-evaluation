...
function controls_visibility(numElements) {
  ...
  if (numElements == 2) {
    thisTool.find('#delete-marker').parent().removeClass("hide");
    thisTool.find('#add-marker').parent().removeClass("hide");
  }
  if (numElements > 2) {
    thisTool.find('#delete-marker').parent().removeClass("hide");
    thisTool.find('#add-marker').parent().addClass("hide");
  }
  if (numElements < 2) {
    thisTool.find('#delete-marker').parent().addClass("hide");
    thisTool.find('#add-marker').parent().removeClass("hide");
  }
}
...
function display_definition() {
  ...
  if ($self.hasClass('dd')) {
    id = $self.prev().val();
  } else if ($self.attr('header')) {
    id = $self.attr('header');
  } else if ($self.attr('id')) {
    id = $self.attr('id');
  } else {
    id = ($self.attr('class')).replace('define','').replace('table_data','').replace('bold','').trim();
  }
